...
{
	/**
	 * Checks if this user_id already has a parent account
	 ...
	 * @todo check if sql statement works !!(it doesnt -- revert back to the logic of processing two queries)
	 */
	...
	public static function getConferenceTitleFromSubAccount( $subAccountId )
	{
		...
		return isset($resultRow) ? $resultRow->page_title : null;
	...
	public static function getSubAccountFromParentReg( $regId )
	{
		if ( $regId )
		{
			$dbr = wfGetDB( DB_SLAVE );
			$res = $dbr->selectRow( 'page_props',
					'*',
					array( 'pp_page' => $regId, 'pp_propname' => 'cvext-registration-account' ) );
		}
		return isset($res) ? $res->pp_value : null;
	}
